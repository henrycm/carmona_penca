...
<body>
	...
	<form action="../administracion/BuscadorAfiliado" name="buscarAfiliado"
		method="get">
		<input type="hidden" name="accion" value="A" /> <input type="hidden"
			name="actualizar" /> <input type="hidden" name="documento" />

		<table width="50%" class="tabla">
			<tr>
				<td width="173">Buscar Distribuidor</td>
				<td width="330"><table width="200">
						<tr>
							<td><a
								href="javascript:if(asignarDocumento()){buscarDistribuidorLiquidacion();}">
									Consultar </a></td>
						</tr>
					</table> &nbsp;</td>
			</tr>
			...
			<table width="100%" border="1" class="tabla">
				...
				<c:forEach var='Afiliado' items='${listaAfiliados}'>
					<tr>
						...
						<c:if test="${accion=='D'}">
							<td><a
								href="../compensacion/ListarAfiliadosPagoPremiacion?accion=A&cedula=<c:out value='${Afiliado.cedula}'/>"
								target="mainFrame">Seleccionar</a></td>
						</c:if>
						<c:if test="${accion=='E'}">
							<td><a
								href="../compensacion/ListarAfiliadosPagoCompensacion?accion=A&cedula=<c:out value='${Afiliado.cedula}'/>"
								target="mainFrame">Seleccionar</a></td>
						</c:if>
					</tr>
				</c:forEach>
