...
<c:if test='${afiliado!=null}'>
	<body
		onload="javascript:seleccionarListaDesplegable('<c:out value="${afiliado.departamento}"/>',document.forma.departamento);">
</c:if>
<c:if test='${afiliado==null}'>
	<body>
</c:if>
<div align="center">
	<form name="forma" action="AfiliadoFrontController" method="post">
		<div align="center" class="titulo">CONSULTA DE DATOS DE AFILIADO
		</div>
		<div align="left">
...
Código Nuevo Empresario: <input name="codigoEmpresario" type="text"
			...
			<c:if test='${afiliado!=null}'>
Código Nuevo Empresario: <input name="codigoEmpresario" type="text"
					size="15" value="<c:out value='${afiliado.cedula}'/>"
					readonly="readonly" />
				<a href="javascript:actualizarAfiliadoADistribuidor();">Actualizar
					A Distribuidor</a>
		...
		<div class="demo">
			<c:if test='${afiliado!=null}'>
				<div id="accordion">
					...
					<div id="datosPersonales">
						<table width="100%">
							<tr align="left">
								...
								<td width="25%">Fecha de Nac: <c:out
										value='${afiliado.fechaNacimiento}' /></td>
							</tr>
							<tr align="left">
								<td>Lugar de Nac.: Ciudad <c:out value='${afiliado.ciudad}' />
								</td>
								<td height="26">Dpto <c:out
										value='${afiliado.departamento}' />
								</td>
								<td>Profesion u Ocupación:<c:out
										value='${afiliado.ocupacion}' /></td>
								<td width="18%">Estado Civil: <c:out
										value='${afiliado.estadoCivil}' />
							</tr>
							<tr align="left">
							...
							<tr align="left">
								...
								<td>Celular o Beeper: <c:out value='${afiliado.celular}' />
								</td>
								<td>E-mail: <c:out value='${afiliado.email}' /></td>

								<td width="21%" height="24" align="left">Nombre del
									Conyugue/Tutor: <c:out value='${afiliado.nombreConyugue}' />
								</td>
							</tr>
						</table>
					</div>
					...
					<h3>
						<a href="#">DATOS DEL PATROCINADOR</a>
					</h3>
