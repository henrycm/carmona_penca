...
public class ReporteCompensacionAfiliadoPeriodo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (vd1 != null) {
				if (afiliado != null) {
					List<Object> lista = this.compensacionAfiliadoService.consultar(documento,
							periodo);
					if ((lista != null) && (lista.size() > 0)) {
						map.put("nombreAfiliado",
						...
						if ("PDF".equals(tipoReporte)) {
							GenerarReporte
									.exportarPDF(request, response, getServletConfig()
											.getServletContext(), "listaCompensacion_" + documento
											+ "_" + periodo + ".pdf",
											RecursosEnum.FW_JASPER_LISTA_COMPENSACION_AFILIADO
													.getRecurso(), map, lista);
						} else {
							GenerarReporte
									.exportarExcel(request, response, getServletConfig()
											.getServletContext(), "listaCompensacion_" + documento
											+ "_" + periodo + ".xls",
											RecursosEnum.FW_JASPER_LISTA_COMPENSACION_AFILIADO
													.getRecurso(), map, lista);
						...
						request.setAttribute("error",
								"El afiliado no ha ganado compensacion por sus consumos para el periodo "
										+ periodo + ".");
 ...
 * 
 * Qualified Name:
 * co.com.multinivel.liquidacion.ReporteCompensacionAfiliadoPeriodo
 * 
 * 
 */