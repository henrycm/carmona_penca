...
public class PedidoDAOImp implements PedidoDAO {
	...
	public boolean ingresarPedido(Pedido pedido) throws MultinivelDAOException {
		try {
			...
			Query rs = this.entityManager
					.createNativeQuery("SELECT SALDO FROM t_saldos_pedido_distribuidor where  distribuidor=?");
			...
			if (saldo != null) {
				Query rs1 = this.entityManager
						.createNativeQuery(" UPDATE t_saldos_pedido_distribuidor SET saldo=? WHERE    distribuidor=? ");
				long saldo2 = (saldo.longValue() < 0L ? 0L : saldo.longValue())
						+ pedido.getTotalPedido().longValue();
				rs1.setParameter(1, Long.valueOf(saldo2));
				rs1.setParameter(2, pedido.getDistribuidor());
				rs1.executeUpdate();
			} else {
				Query rs1 = this.entityManager
						.createNativeQuery("INSERT INTO t_saldos_pedido_distribuidor(distribuidor,saldo)values(?,?)");
	...
	public List<Object> consultar(Pedido pPedido) throws MultinivelDAOException {
		...
		try {
			...
			for (int i = 0; i < s; i++) {
				...
				String fecha = ((java.sql.Date) objectArray[13]).toString();
	...
	public boolean actualizar(Pedido pedido) throws MultinivelDAOException {
		...
		try {
			Pedido pedidoConsultado = (Pedido) this.entityManager.find(Pedido.class,
					Integer.valueOf(pedido.getCodigoPedido()));
	...
	public Pedido buscar(Pedido pedido) throws MultinivelDAOException {
		return (Pedido) this.entityManager.find(Pedido.class,
				Integer.valueOf(pedido.getCodigoPedido()));
	}
		...
		try {
			...
			if (query.getResultList() != null) {
				...
				for (int i = 0; i < s; i++) {
					...
					BigDecimal transporte = (BigDecimal) objectArray[1];
					BigDecimal totalPedido = (BigDecimal) objectArray[2];
					java.util.Date fecha = (java.util.Date) objectArray[3];
					String codigoProducto = (String) objectArray[4];
					String nombreProducto = (String) objectArray[5];
					BigDecimal valorUnitario = (BigDecimal) objectArray[6];
	...
	public boolean eliminarPedido(Pedido pedido) throws MultinivelDAOException {
		try {
			Query rs3 = this.entityManager
					.createNativeQuery(" DELETE FROM T_DET_PEDIDOS  WHERE codigo_pedido=? ");
			rs3.setParameter(1, Integer.valueOf(pedido.getCodigoPedido()));
			rs3.executeUpdate();

			Query rs2 = this.entityManager
					.createNativeQuery(" DELETE FROM T_PEDIDOS  WHERE codigo_pedido=? ");
