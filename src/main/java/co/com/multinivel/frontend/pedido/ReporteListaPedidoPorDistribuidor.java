		...
		try {
			...
			if ((lista != null) && (lista.size() > 0)) {
				...
				if ("PDF".equals(tipoReporte)) {
					GenerarReporte.exportarPDF(request, response, getServletConfig().getServletContext(), "Reporte_ListaPedidosDistribuidor_"
							+ periodo + ".pdf", RecursosEnum.FW_JASPER_REPORTE_LISTA_PEDIDOS_DISTRIBUIDOR.getRecurso(), map, lista);
				} else {
					GenerarReporte.exportarExcel(request, response, getServletConfig().getServletContext(), "Reporte_ListaPedidosDistribuidor_"
							+ periodo + ".xls", RecursosEnum.FW_JASPER_REPORTE_LISTA_PEDIDOS_DISTRIBUIDOR.getRecurso(), map, lista);
				}
			} else {
 ...
 * 
 * Qualified Name:
 * co.com.multinivel.frontend.pedido.ReporteListaPedidoPorDistribuidor
 */